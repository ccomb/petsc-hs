...
data NZPR
  = ConstNZPR (DNZ, ONZ)
  | VarNZPR (VS.Vector DNZ, VS.Vector ONZ)
  deriving (Eq, Show)
...
type IdxRow = Int
type IdxCol = Int
newtype MatrixDataZ a
  = MatrixDataZ {matDataZ :: V.Vector (IdxRow, IdxCol, a)}
  deriving (Eq, Show)
...
checkMatrixData (MatrixDataZ z) = (lr == lc) && (lr == le)
 where
...
matSeqAIJSetPreallocationConstNZPR mat nz = chk0 (matSeqAIJSetPreallocationConstNZPR' mat nz')
 where
  nz' = toCInt nz

matSeqAIJSetPreallocationVarNZPR :: Mat -> VS.Vector Int -> IO ()
matSeqAIJSetPreallocationVarNZPR mat nnz = chk0 (matSeqAIJSetPreallocationVarNZPR' mat nnz')
 where
...
matCreateVecLeft m = VecLeft <$> chk1 (matCreateVecLeft' m)

-- vector that the matrix can be multiplied against
matCreateVecRight :: Mat -> IO VecRight
matCreateVecRight m = VecRight <$> chk1 (matCreateVecRight' m)
...
matGetRow mat ro = do
  ...
  colsv <- getIVG n cols
  valsv <- getVG n vals
  return $ VG.zip colsv valsv
 where
  r = toCInt ro

matRestoreRow :: Mat -> Int -> IO ()
matRestoreRow m ro = chk0 (matRestoreRow0Safe' m r)
 where
  r = toCInt ro
...
validDims mi (VarNZPR (dnnz, onnz)) =
  validDims' mi
    ...
    VG.all withinCols dnnz
    && VG.all withinCols onnz
 where
...
listToCSR m n ll
 ...
 where
  ii = V.fromList $ concatMap (replicate n) [0 .. m - 1]
