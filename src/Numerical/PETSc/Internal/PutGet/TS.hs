...
tsSetIFunction ::
  ...
  IO ()
tsSetIFunction ts res f = tsSetIFunction_ ts res g
 where
...
tsSetIJacobian ::
  ...
  IO ()
tsSetIJacobian ts amat pmat fun = tsSetIJacobian_ ts amat pmat fun'
 where
...
tsSetRHSFunction ::
  ...
  IO ()
tsSetRHSFunction ts r f = tsSetRHSFunction_ ts r g
 where
...
tsSetRHSJacobian ::
  ...
  IO ()
tsSetRHSJacobian ts amat pmat f = tsSetRHSJacobian_ ts amat pmat g
 where
...
tsSetCostGradients ts numcost lambda_ mu_ =
  withArray lambda_ $ \lp ->
    withArray mu_ $ \mp ->
      chk0 $ tsSetCostGradients' ts n lp mp
 where
...
tsAdjointSetRHSJacobian ts amat f =
  chk0 $ tsAdjointSetRHSJacobian0' ts amat g
 where
