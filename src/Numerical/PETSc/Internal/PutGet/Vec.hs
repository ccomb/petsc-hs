...
import qualified Data.Ix as Ix (inRange, range)
...
vecCreateMPIInfo vi = vecCreateMPI co nl ng
 where
...
withVecCreate vv = withVec (vecCreate c)
 where
  c = vecInfoMpiComm vv
...
vecSetValuesRangeVector v =
  vecSetValuesUnsafeVector v ix_
 where
...
vecOverwriteIOVectorN2_ v vnew
 ...
 where
  (lv, lvnew) = (vecSize v, V.length vnew)
