...
traverseFilter_ p mf = V.mapM_ mg
 where
  mg x
    | p x = mf x
    | otherwise = return ()
...
inBoundsUnsortedList lx (x1, x2)
  | validX = s
  | otherwise = error "inBoundsUnsortedList : invalid bounds x1 >= x2 "
 where
  (l1, l2) = extremaUnsortedList lx
  s = l1 >= x1 && l2 <= x2
  validX = x1 < x2

extremaUnsortedList lx = (head lxs, last lxs)
 where
  lxs = qsort lx
...
linspace1a n a b = mv
 where
  ...
  mv = zipWith (\x y -> 1 / 2 * (x + y)) mv1 mv2
  dt = (b - a) / fromIntegral n
